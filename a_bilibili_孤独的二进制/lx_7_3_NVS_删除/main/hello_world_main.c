...
uchar key_read(void)
{
    if (gpio_get_level(KEY) == 0) // 按键按下(0为低电平按下,1为高电平松开)
    {
        vTaskDelay(10 / portTICK_PERIOD_MS); // 延时10毫秒,消抖
        if (gpio_get_level(KEY) == 0)        // 按键按下(0为低电平按下,1为高电平松开)
        {
            while (gpio_get_level(KEY) == 1)
                ;     // 松手检测。当gpio_get_level(KEY) == 0时，按键控制LED取反
            return 1; // 有按键按下
        }
    }
    return 0;
...
void app_main()
{
    ...
    else
    {
        ...
        if (key_read())
        {
            // 先提交再擦除再关闭。如果在调用 nvs_commit 函数之前，实际存储可能不会更新。

            // 擦除具有给定键名称的键值对
            err = nvs_erase_key(my_handle, str_key);
            if (err != ESP_OK)
                ESP_LOGE(TAG, "nvs_erase_key Error");
            else
                ESP_LOGE(TAG, "nvs_erase_key success");
