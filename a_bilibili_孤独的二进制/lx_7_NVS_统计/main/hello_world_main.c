...
void app_main()
{
    ...
    // printf("Count: UsedEntries = (%d), FreeEntries = (%d), AllEntries = (%d), Namespace_count = (%d)\n",
    ...
    // 计算命名空间内的存储数量
    // 如果键值为整数型，条目数量即为 1。如果键值为字符串或BLOB类型，则条目数量取决于值的长度。
    size_t used_entries;            // 该命名空间的存储条目数量
    size_t total_entries_namespace; // 该命名空间的总条目数量。因为名称空间条目占用一个条目，所以要在返回值 used_entries 加一
    if (nvs_get_used_entry_count(my_handle, &used_entries) == ESP_OK)
    {
        // the total number of entries occupied by the namespace
        total_entries_namespace = used_entries + 1;
        printf("%s my_handle_used_entries is %d\n", TAG, used_entries);
        printf("%s my_handle_total_entries_namespace is %d\n", TAG, total_entries_namespace);
    }
